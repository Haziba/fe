...
console.log('inside oauth auth');
  ...
  if (callbackURL) {
    console.log('inside callback', callbackURL);
  ...
  if (req.query && req.query.code) {
    ...
    function loaded(err, ok, state) {
      console.log('inside loaded', err, ok);
...
console.log('get oauth access token');
      self._oauth2.getOAuthAccessToken(code, params,
        function(err, accessToken, refreshToken, params) {
          console.log('inside get oauth access token', err);
...
console.log('skip query bit');
    var state = req.query.state;
    try {
      var arity = this._stateStore.verify.length;
      console.log('try arity', arity);
      if (arity == 4) {
    ...
    if (state) {
      ...
      var location = this._oauth2.getAuthorizeUrl(params);
        console.log('get auth url', location);
      this.redirect(location);
    } else {
      function stored(err, state) {
        console.log('stored called', err, state);
        if (err) { return self.error(err); }
      ...
      try {
        ...
        if (arity == 3) {
          ...
          console.log(this._stateStore.store, stored);
